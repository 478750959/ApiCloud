...
function ajaxToAPICloud(appId, appKey, method, classInfo, callback, filter){
    /* 给部分参数设置默认值;同时转换为小写,以使下面的代码更加灵活.*/
    var method = (method || "get").toLowerCase();

    /* 用于向外部传递值的回调函数. */
    function cbBlock(ret, err){
    ...
    /* 先检查下有没有必须的信息. */
    if( ! (appId && appKey && classInfo && classInfo["className"])){
        var ret = undefined;
        var err = {
            msg: "appId, appKey 或classInfo 不能为空."
        };
        cbBlock(undefined, err);
    ...
    ajax(appId, encryptKey, method, classInfo, cbBlock, filter);

    // ----------------下面的代码不要乱动,除非你非常清楚自己在做什么!---------------------
     ...
     *
     * appId        string    应用ID.在网站控制台-->应用概览界面获取.
     * encryptKey   string    加密后的特定密钥.encryptKey = sha1(appId + 'UZ' + appKey + 'UZ' + 当间时间毫秒数）.当间时间毫秒数
     * method       string    操作类型,不区分大小写,默认get.可选get(查询),post(新建),put(更新),delete(删除)
     * classInfo    object    类的信息.内部字段:
     *              {
     *                 className: "MyClass", // 类的名称.必选.
     ...
     *                            value: "", // 列的值. file 类型的字段仅需给出文件路径名即可.
     *                            type: "file", // 列的类型,不区分大小写.当列的类型为 file 时必须特别指明.
     *                            filename: "文件名" // 文件名.可选.仅当列的类型为 file 时,此字段才有意义.
     *                           }] // 包各列字段信息的一个数组.
     *
     *              }
     * callback     function   方法执行成功或失败时的回调函数.内部字段:
    ...
    function ajax(appId, encryptKey, method, classInfo, callback, filter){
        var  baseUrl = "https://d.apicloud.com/mcm/api/";

        var url = baseUrl + classInfo.className;

        /* 当为post(创建)和put(更新)操作时,需要先对 file 字段做一次转换 */
        ! function(appId, encryptKey, method, classInfo, callback, filter){
            var selfFunc = arguments.callee;

            var fileField; // 尚未处理的文件字段.每次只处理一个.
            ...
            if(("post" == method ||  "put" == method) &&
                ...
                for(var idx in fields){
                    ...
                    if("file" === field.type.toLowerCase() &&
                        "string" === typeof  field.value){
                        fileField = field;
            ...
            if(fileField){
                ...
                api.ajax({
                    ...
                    classInfo.fields[fileField.name] = ret;
                    var fields = classInfo["fields"]
                    for(var idx in fields){
                        var field = fields[idx];
                        ...
                        if(fileField.name === field.name){
                            field.value = ret;

                            selfFunc(appId, appKey, method, classInfo, callback, filter);
                            break;
                        }
                    }
                });
        ...
        }(appId, encryptKey, method, classInfo, function(classInfo){
            /* 如果存在字段信息,则转换为JSON对象,以便于传输. */
            var body = {};
            ...
            if(classInfo["fields"]){
                ...
                for(var idx in fields){
                    ...
                    if(undefined === field.name || undefined === field.value){
                        continue;
                    }
            ...
            /* 当为 get查询请求时,提供对过滤器的支持. */
            if("get" == method && "string" === typeof filter){
                url += "?" + filter;
            }
        ...
        /* 最关键的部分: 通过 api.ajax 与数据库进行交互.
         *
         * appId        string    应用ID.在网站控制台-->应用概览界面获取.
         * encryptKey   string    加密后的特定密钥.encryptKey = sha1(appId + 'UZ' + appKey + 'UZ' + 当间时间毫秒数）.当间时间毫秒数
         * method       string    操作类型,默认get.可选get(查询),post(新建),put(更新),delete(删除)
         * url          string    请求的restfule风格接口的地址.
         * body         object    请求体.
         * callback     function   方法执行成功或失败时的回调函数.内部字段:
         *              {
         *                ret: {
